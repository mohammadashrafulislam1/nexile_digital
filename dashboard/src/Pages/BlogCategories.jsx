...
import { endPoint } from '../Components/ForAll/ForAll'; // Ensure this path is correct
...
const BlogCategories = () => {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${endPoint}/blogCategory`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isUpdating) {
        await axios.put(`${endPoint}/blogCategory/${currentId}`, { name });
      } else {
        await axios.post(`${endPoint}/blogCategory`, { name });
      }
      Swal.fire({
        position: "top-end",
        icon: "success",
        title: `${isUpdating ? "Blog Category updated successfully!" : "Blog Category added successfully!"}`,
  ...
  return (
    <div className="my-10 p-6">
        ...
        <ul className="my-6">
          ...
          <li>
            <span className="inline-flex items-center gap-2">
              {/* SVG icon for Manage Blog Categories */}
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="h-4 w-4 stroke-current">
      ...
      <h2 className="text-2xl font-semibold text-center mb-4">{isUpdating ? 'Update Blog Category' : 'Add Blog Category'}</h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-5 lg:w-1/2 md:w-[80%] w-[96%] mx-auto">
        ...
        <button type="submit" className="w-full mt-5 btn btn-sm bg-black text-white font-semibold py-2 rounded-md transition duration-200">
          {isUpdating ? 'Update Blog Category' : 'Add Blog Category'}
        </button>
      </form>

      {/* List of blog categories */}
      {categories?.length > 0 ? (
        <h2 className="text-xl font-semibold mt-10">Blog Categories</h2>
      ) : (
